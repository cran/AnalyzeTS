    ...
    r = 13, CEF = "MSE", type = "Abbasov-Mamedova", show.complete = TRUE) 
{
    ...
    if (CEF != "ME" & CEF != "MAE" & CEF != "MPE" & CEF != "MAPE" & 
        CEF != "MSE" & CEF != "RMSE") 
        stop("Error in 'CEF'!")
    if (show.complete != 0 & show.complete != 1) 
    ...
    thamso.mohinh <- data.frame(thamso.n, thamso.w, thamso.D1, 
        thamso.D2, thamso.error, thamso.k, thamso.r, thamso.CEF, 
        thamso.type, giatri.C, giatri.CEF)
    id = 0
    ...
    for (i in 1:so.mohinh) {
        tukhoa <- paste("Adidaphat", so.mohinh, "Adidaphat", 
            i, "Adidaphat", sep = "")
        C.best <- DOC(ts, n = thamso.mohinh[i, 1], w = thamso.mohinh[i, 
            2], D1 = thamso.mohinh[i, 3], D2 = thamso.mohinh[i, 
            4], error = thamso.mohinh[i, 5], k = thamso.mohinh[i, 
            6], r = thamso.mohinh[i, 7], CEF = as.character(thamso.mohinh[i, 
            8]), type = as.character(thamso.mohinh[i, 9]), show.complete = show.complete, 
            keyword = tukhoa)
        thamso.mohinh[i, 10:11] <- C.best
    }
    if (sum(thamso.mohinh[, 9] == "Abbasov-Mamedova") > 0) {
        n1 <- thamso.mohinh[thamso.mohinh[, 9] == "Abbasov-Mamedova", 
            1]
        w1 <- thamso.mohinh[thamso.mohinh[, 9] == "Abbasov-Mamedova", 
            2]
        C1 <- thamso.mohinh[thamso.mohinh[, 9] == "Abbasov-Mamedova", 
            10]
        CEF1 <- thamso.mohinh[thamso.mohinh[, 9] == "Abbasov-Mamedova", 
            11]
        datC.best1 <- data.frame(n1, w1, C1, CEF1)
        colnames(datC.best1) <- c("n parameter", "w parameter", 
            "C value", paste(CEF, "value"))
    }
    if (sum(thamso.mohinh[, 9] == "Abbasov-Mamedova") == 0) 
        datC.best1 <- NULL
    if (sum(thamso.mohinh[, 9] == "NFTS") > 0) {
        ...
        colnames(datC.best2) <- c("n parameter", "w parameter", 
            "C value", paste(CEF, "value"))
    }
    if (sum(thamso.mohinh[, 9] == "NFTS") == 0) 
        datC.best2 <- NULL
